...
from util.tune.objective.create_RandomForestClassifier import (
    create_RandomForestClassifier,
    params_const,
)
...
# 定義目標函數
def objective(trial):
    model = create_RandomForestClassifier(trial)
    return cross_val_10_fold(model, X_train, y_train)


study = optuna.create_study(
    study_name="random-forest-without-llm",
    direction="maximize",
...
model = RandomForestClassifier(**params_const, **best_params)
