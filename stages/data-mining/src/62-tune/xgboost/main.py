...
from util.tune.objective.create_XGBClassifier import (
    create_XGBClassifier,
    params_const,
)
...
def objective(trial):
    model = create_XGBClassifier(trial)
    return cross_val_10_fold(model, X_train, y_train)


# Create a study and optimize the objective function
study = optuna.create_study(
    study_name="xgboost-without-llm",
    direction="maximize",
...
model = XGBClassifier(**params_const, **best_params)
