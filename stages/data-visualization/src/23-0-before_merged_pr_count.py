...
# df = df[df["before_merged_pr_count"] > 0]
df = df[df["before_merged_pr_count"] <= 30]

binwidth = 1
bin_edges = np.arange(0, df["before_merged_pr_count"].max() + binwidth, binwidth)
...
output_path = "images/23-0-before_merged_pr_count.png"
