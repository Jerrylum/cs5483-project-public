...
# Filter out rows where changed_files > 20
df = df[df["changed_files"] > 0]
df = df[df["changed_files"] <= 20]

# Show numbers of changes_quality group by final_state
print(df.groupby(["changed_files", "final_state"]).size())

# Group by 'nature' and 'final_state', then normalize to percentages
nature_counts = df.groupby(["changed_files", "final_state"]).size().unstack()
...
# Sort the DataFrame index (changed_files) in descending order
...
ax = nature_percentages.plot(
    kind="barh",
    stacked=True,
    title="changed_files Distribution (Percentage) (<= 20)",
    color=custom_colors,
...
# output_path = "images/06-1-changed_files-percentage.png"
