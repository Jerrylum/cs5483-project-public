...
print(df.groupby(["comments", "final_state"]).size())

# Group by 'nature' and 'final_state', then normalize to percentages
nature_counts = df.groupby(["comments", "final_state"]).size().unstack()
...
# Sort the DataFrame index (comments) in descending order
...
ax = nature_percentages.plot(
    kind="barh",
    stacked=True,
    title="Comments Distribution (Percentage)",
    color=custom_colors,
...
output_path = "images/02-1-comments-percentage.png"
